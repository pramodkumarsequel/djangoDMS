...
$(document).ready(function(){
    ...
    jQuery('.clear-hierarchy5-filter').click(function(){
        window.location.href = "/hierarchy5-list/";
    });
...
$("#id_hierarchy1").change(function () {
  ...
  $("#id_hierarchy3").change(function () {
    var url = $("#itmForm").attr("data-hier4-url");  // get the url of the `load_cities` view
    var stateId = $(this).val();  // get the selected country ID from the HTML input

    $.ajax({                       // initialize an AJAX request
      url: url,                    // set the url of the request (= localhost:8000/hr/ajax/load-cities/)
      data: {
        'hierarchy3': stateId       // add the country id to the GET parameters
      },
      success: function (data) {   // `data` is the return of the `load_cities` view function
        $("#id_hierarchy4").html(data);  // replace the contents of the city input with the data that came from the server
      }
    });
